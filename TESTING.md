...
| B | The song is heard | **Automated, queued** | The game itself must have the tree's song among its playing one-shots, under the def the modlist calls for (`03`, "the game plays the tree's song when the first listener sits"): filed 2026-09-25, **not passed yet** (the order was refused twice, see `docs/runs/`). That the audio output renders it: `PickleTools/SoundCapture` runs **in the WSL like any test** since the owner withdrew the "Windows only" rule on 2026-09-25 (`AUDIT.md`, ligne 8; its README of the same day): the WSL profile mutes the game, so the scenario sets the volume first, and the sound also plays on her speakers for a few seconds. `04-the-song-heard.feature` records it, and a video with its sound (`film-sound.mp4`) is what she can watch and listen to. Not written for the film yet |
...
| Sound recording | `-Mod AnimaSong -DepMap wsl-deps.sound.map`, small ticket, in the WSL | `04-the-song-heard.feature` (`@requires:nelim.pickletools.soundcapture`, skipped in every other pass): the sink is recorded while the first listener walks out and sits (volume set first, the WSL profile mutes the game), the level is measured, and `sound.wav` is kept beside the report. The sound plays on the owner's speakers for those seconds |
