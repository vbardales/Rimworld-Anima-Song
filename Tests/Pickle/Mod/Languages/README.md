...
This folder is not a translation. It exists so that RimWorld stops logging
    ...
    Mod Anima Song - Pickle tests did not load any content.
...
DefInjected files are loaded for real, and this companion stops being inert — which would matter
here more than elsewhere: this suite's whole point is to photograph the mod's own text in the
language the pass runs in.
