...
| `AUDIT.md` | `90d51374` 2026-09-25, clean | The whole chain. Rules that changed what I did: **"test manuel" = what is left to tick by hand must be automated and green, or listed non applicable with its reason** (step 9), not "things the owner plays"; **fail fast**: no red without a green replay, the gallery and the owner's manual validations before the `publish`, the rollback target chosen beforehand; **SoundCapture tests are played alone, on Windows, only on the owner's explicit request** (`Run-Windows.ps1`, under the lock), never through the WSL queue; **a request carries no SHA, so the mod tree stays frozen until `RUN_DONE` and the SHA goes in `-Label`**; no watcher, no `Monitor`, no cron; session title `<mod> / <stage>` |
